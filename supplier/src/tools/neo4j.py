...
class Neo4jGraphDB:
    ...
    def create_node(self,node:StructuredNode):
        ...
        try:
            ...
            logging.error(f'can not create node with name {self.node.name}')
            logging.error(e)
    ...
    def create_relationship_to(self,node:StructuredNode,relationship):
        ...
        try:
            ...
            logging.error(f'can not creeate relationship {relationship} for node {node.name}')
            logging.error(e)

    def add(self, node: StructuredNode):
        ''' Add data to a particular node, create if not exists, update if exists '''
        try:
            # Check if node with the same unique identifier (e.g., supplier_id) exists
            ...
            if existing_node:
                ...
                logging.info(f"Node with {node.supplier_id} updated.")
                return existing_node
            else:
                # If node does not exist, create it
                node.save()  # Create new node
                logging.info(f"Node with {node.supplier_id} created.")
                return node

        except RuntimeError as e:
            logging.error(f"Error adding data to node: {node.supplier_id}")
            logging.error(e)
            return None
    ...
    def _get_existing_node(self, node: StructuredNode):
        ...
        try:
            # Replace supplier_id with the actual unique identifier for your use case
            return node.__class__.nodes.get(supplier_id=node.supplier_id)
    ...
    def get(self,node:StructuredNode):
        ...
        try:
            ...
            logging.error(f'getting data from from node')
            logging.error(e)
