...
def create_shipping_provider_nodes_in_knowledge_graph_helper(provider_id:str):
    prompt_template = PromptTemplate(input_variables=["parameters"], template="""
        ...
        1️. Retrieve Shipping Provider
        - **Step:** 1
        - **Action:** GetShippingProviderDetails (parameters: {parameters})

        2️. Retrieve Shipping Provider Metadata
        - **Step:** 2
        - **Action:** GetShippingProviderMetadata (parameters: {parameters})
        
        3. Add a new Shipping Provider
        - **Step:** 6 
        - **Action:** AddShippingProvider (parameters: {parameters})                              
    ...
    parameters_json = json.dumps({"provider_id": provider_id})
    ...
    for step in steps:
        print(step.keys())
        if "parameters" in step.keys() and "provider_id" not in step.keys():
